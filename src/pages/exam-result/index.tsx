          ...
          <>
            <ScrollView scrollY className="result-scroll">
              <View className={`result-hero ${data.passed ? 'gradient' : 'failed-gradient'}`}>
                <Text className="result-status"><Icon name={data.passed ? 'VerifiedCheck' : 'X'} /> {data.resultStatusText}</Text>
                ...
                {data.passed ? <View className="hero-action" onClick={() => Taro.navigateTo({ url: `/pages/certificate-detail/index?certificateId=${data.certificateId || 99}` })}><Icon name="Medal" /> 查看证书</View> : null}
              </View>
              <View className="summary-grid">
                {data.dataOverview.map(item => (
                  <View className="summary-card soft-card" key={item.name}>
              ...
              <View className="section-card soft-card">
                <View className="section-head"><Text className="section-title">题型表现</Text><Text className="section-link" onClick={() => Taro.showToast({ title: '敬请期待', icon: 'none' })}>查看错题</Text></View>
              ...
              <View className="tip-card soft-card">
                <Text className="tip-title">{data.tipInfo.title}</Text>
                <Text className="tip-desc">{data.tipInfo.desc}</Text>
              </View>
            ...
            <View className="fixed-bottom-actions">
              <View
                ...
                onClick={() => {
                  ...
                  Taro.setStorageSync('course_list_pending_category', 'required');
                  Taro.switchTab({ url: '/pages/course-list/index' });
                }}
              >
                {data.passed ? '返回首页' : '去学习'}
              </View>
              <View className="primary-btn" onClick={() => Taro.redirectTo({ url: retryUrl })}>{data.passed ? '再练一次' : '重新考试'}</View>
            </View>
          </>
        )}
      </View>
